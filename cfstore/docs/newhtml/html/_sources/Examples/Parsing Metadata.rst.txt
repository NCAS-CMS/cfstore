...
Further information on what b-metadata entails in available here: 

    https://www.dcc.ac.uk/news/bryan-lawrence-metadata-limit-sustainability
...
In addition to this, cfdb can parse metadata from aggregation files also provides a tool to read an aggregation file and store the metadata in a collection
This uses the (A)dd (A)ggregation (F)ile (T)o (C)ollection (aaftc) function::
  
    cfdb aaftc <aggregationfile> <collection>
