...
import nlist
import utils
from jdma_client import jdma_common, jdma_lib
...
class JDMAInterface:
    ...
    def submit_migrate(self, filelist):
        ...
        batch_id = self._get_batch_id_for_path(
            path
        )  # utils.log_msg('batch id: {}'.format(batch_id), level='INFO')

        if batch_id != None:
            # raise JDMAInterfaceError(('Path {} has already been migrated (as batch ID: {})'
            #                          ).format(path, batch_id))
            utils.log_msg(
                "Path {} has already been migrated (as batch ID: {})".format(
                    path, batch_id
                ),
                level="ERROR",
            )
    ...
    def _resp_to_req_id(self, resp):
        ...
        if status_code == 200:
            ...
            utils.log_msg(
                "JDMA request failed with HTTP status code {} and message: {}".format(
                    status_code, fields["error"]
                ),
                level="ERROR",
            )
        else:
            utils.log_msg(
                "JDMA request failed with HTTP status code {}".format(status_code),
                level="ERROR",
            )
    ...
    def _get_batch_id_for_path2(self, path):
        ...
        resp = jdma_lib.get_batch(self.username, workspace=workspace, label=path)

        if resp.status_code != 200:
            if resp.status_code % 100 == 5:
                utils.log_msg(
                    (
                        "Warning: JDMA responded with status code {} when checking for "
        ...
        if num_matches == 0:
            ...
            utils.log_msg(
                "found more than one batch on storage for path {} (ids={})".format(
                    path, ",".join(map(str, batch_ids))
                ),
                level="ERROR",
            )
...
class Jasmin(object):
    ...
    def __init__(self, input_nl="jasmin.nl"):
        ...
        try:
            ...
            msg = (
                "Transfer: Failed to load &jasmin_arch "
                "namelist from namelist file: " + input_nl
            )
            utils.log_msg(msg, level="FAIL")
