...
class Transfer(object):
    ...
    def __init__(self, input_nl="pptransfer.nl"):
        ...
        try:
            ...
            msg = (
                "Transfer: Failed to load &archer_arch "
        ...
        try:
            ...
            msg = (
                "Transfer: Failed to load &pptransfer "
                "namelist from namelist file: " + input_nl
            )
            utils.log_msg(msg, level="FAIL")
    ...
    def tidy_up(self):
        ...
        if self._transfer_type == "push":
            if os.path.exists(self._archive_dir):
                ...
                msg = "Archive directory {} doesn't exist".format(self._archive_dir)
            ...
            if ret_code == 0:
                ...
                msg = "Remote archive directory {} doesn't exist".format(
                    self._archive_dir
                )
                utils.log_msg(msg, level="ERROR")
    ...
    def _clean_up_pull(self):
        ...
        if ret_code == 0:
            ...
            # the remote host..
            msg = "Problem checking existence of checksum files on " "remote host"
            utils.log_msg(msg, level="WARN")
    ...
    def _generate_checksums(self):
        ...
        if self._transfer_type == "push":
            ...
            ret_code, output = utils.exec_subproc(
                cmd, verbose=False, cwd=self._archive_dir
            )
            with open(os.path.join(self._archive_dir, self._checksums), "w") as csfh:
                csfh.write(output)
        else:
            # Pulling files
    ...
    def do_transfer(self):
        ...
        utils.log_msg(
            "Total {0:.1f} Gb of data to transfer".format(gigabytes), level="INFO"
        )
        ...
        if self._gridftp:
            ...
            globus_cmd = "globus-url-copy -vb -cd -r -rst -rst-retries 5 -cc {} {} -sync -cred {}".format(
                self._gridftp_cc_opt, verify, credfile
            )
            if self._transfer_type == "push":
            ...
            if self._transfer_type == "push":
                ...
                transfer_cmd = (
                    "rsync -av --stats "
                    '--rsync-path="mkdir -p {} && rsync" {}/ {}'.format(
                        transfer_dir, archive_dir, remote_host_dir
                    )
                )
            else:
        ...
        put_rtncode = {
            ...
            12: "System Error: Failed to make transfer directory " "(ReturnCode=12)",
        }
