...
class Collection(ProxiedDictMixin, Base):
    ...
    properties = relationship(
        "CollectionProperty", collection_class=attribute_mapped_collection("key")
    )
    _proxied = association_proxy(
        "properties",
        "value",
    ...
    def serialise(self, target="dict"):
        ...
        assert (
            target == "dict"
        ), "Collection can only be serialised to a python dictionary"
        blob = {
            x: getattr(self, x) for x in ["name", "description", "volume", "filecount"]
        }
...
class Tag(Base):
    ...
    in_collections = relationship(
        "Collection", secondary=collection_tags_associations, back_populates="tags"
    )
...
class StorageLocation(Base):
    ...
    holds_files = relationship(
        "File", secondary=storage_files_associations, back_populates="replicas"
    )
...
class File(Base):
    ...
    variables = relationship(
        "Variable", secondary=var_files_associations, back_populates="in_files"
    )
